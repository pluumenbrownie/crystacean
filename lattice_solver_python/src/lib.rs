...
impl BitArrayRepresentation {
    fn solve(&self, find_all: bool) -> Vec<BitArraySolution> {
        self.wrapped.solve(find_all).into_iter()
...
impl Lattice {
    #[new]
    fn python_new(input_lattice: Vec<(Vec<f32>, Vec<Vec<f32>>)>) -> Self {
        Lattice { wrapped: WrappedLattice::python_new(input_lattice) }
    }
    ...
    fn get_intermediary(&self) -> BitArrayRepresentation {
        BitArrayRepresentation{wrapped: self.wrapped.get_intermediary()}
    }

    fn to_solved_lattice(&self, solution: &BitArraySolution) -> Self {
        Self{wrapped: self.wrapped.to_solved_lattice(&solution.wrapped)}
    }
